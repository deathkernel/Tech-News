...
async def fetch_podcasts(client: httpx.AsyncClient) -> list[dict]:
    async def one(query: str):
        try:
            ...
            print(f"[news] Podcast source failed ({type(exc).__name__})", flush=True)
            return []
...
async def fetch_hacker_news(client: httpx.AsyncClient) -> list[dict]:
    try:
        ...
        print(f"[news] Hacker News failed ({type(exc).__name__})", flush=True)
        return []
...
async def fetch_github(client: httpx.AsyncClient) -> list[dict]:
    try:
        ...
        print(f"[news] GitHub source failed ({type(exc).__name__})", flush=True)
        return []
...
async def fetch_nvd(client: httpx.AsyncClient) -> list[dict]:
    try:
        ...
        print(f"[news] NVD source failed ({type(exc).__name__})", flush=True)
        return []
...
async def fetch_arxiv(client: httpx.AsyncClient) -> list[dict]:
    try:
        ...
        print(f"[news] arXiv source failed ({type(exc).__name__})", flush=True)
        return []
...
async def fetch_gdelt(client: httpx.AsyncClient) -> list[dict]:
    try:
        ...
        print(f"[news] GDELT source failed ({type(exc).__name__})", flush=True)
        return []
...
async def fetch_devto(client: httpx.AsyncClient) -> list[dict]:
    try:
        ...
        print(f"[news] DEV Community failed ({type(exc).__name__})", flush=True)
        return []
...
async def fetch_all() -> list[dict]:
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, headers={"User-Agent": USER_AGENT}) as client:
        ...
        if items:
            return items
        # Last-resort live feed: retry Hacker News independently so a transient
        # failure in the parallel batch cannot leave the whole app blank.
