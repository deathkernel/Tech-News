...
async def fetch_feed(source_name: str, feed_url: str, client: httpx.AsyncClient) -> list[dict]:
    try:
        ...
        for entry in parsed.entries[:20]:
            ...
            if title and link: items.append({"title": title, "url": link, "source": source_name, "published_at": published, "summary": entry.get("summary", "").strip(), "image_url": extract_image_url(entry), "image_alt": title})
        return items
    except Exception as exc:
        print(f"[news] RSS source failed: {source_name} ({type(exc).__name__})", flush=True)
        return []
...
async def fetch_podcasts(client: httpx.AsyncClient) -> list[dict]:
    async def one(query: str):
        try:
            ...
            response.raise_for_status(); parsed = feedparser.parse(response.text); result = []
            for entry in parsed.entries[:5]:
                title, summary, link = html.unescape(entry.get("title", "")).strip(), entry.get("summary", "").strip(), entry.get("link", "").strip()
            ...
            print(f"[news] Podcast source failed ({type(exc).__name__})", flush=True); return []
    batches = await asyncio.gather(*(one(query) for query in PODCAST_QUERIES), return_exceptions=True)
...
async def fetch_hacker_news(client: httpx.AsyncClient) -> list[dict]:
    try:
        ...
        for story in stories:
            if not isinstance(story, dict) or story.get("type") != "story" or not story.get("title") or not story.get("url"): continue
            try: published = datetime.fromtimestamp(int(story.get("time", 0)), tz=timezone.utc) if story.get("time") else None
            except (ValueError, TypeError, OSError): published = None
            result.append({"title": html.unescape(str(story["title"])), "url": str(story["url"]), "source": "Hacker News", "published_at": published, "summary": f"Hacker News discussion: {story.get('score', 0)} points, {story.get('descendants', 0)} comments.", "image_url": None, "image_alt": str(story["title"])})
        return result
    except Exception as exc:
        print(f"[news] Hacker News failed ({type(exc).__name__})", flush=True); return []
...
async def fetch_github(client: httpx.AsyncClient) -> list[dict]:
    try:
        ...
        return result
    except Exception as exc: print(f"[news] GitHub source failed ({type(exc).__name__})", flush=True); return []


async def fetch_nvd(client: httpx.AsyncClient) -> list[dict]:
    try:
        now = datetime.now(timezone.utc); start = now - timedelta(days=1)
        ...
        return result
    except Exception as exc: print(f"[news] NVD source failed ({type(exc).__name__})", flush=True); return []
...
async def fetch_arxiv(client: httpx.AsyncClient) -> list[dict]:
    ...
    except Exception as exc: print(f"[news] arXiv source failed ({type(exc).__name__})", flush=True); return []
...
async def fetch_gdelt(client: httpx.AsyncClient) -> list[dict]:
    ...
    except Exception as exc: print(f"[news] GDELT source failed ({type(exc).__name__})", flush=True); return []
...
async def fetch_devto(client: httpx.AsyncClient) -> list[dict]:
    ...
    except Exception as exc: print(f"[news] DEV Community failed ({type(exc).__name__})", flush=True); return []
...
async def fetch_all() -> list[dict]:
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, headers={"User-Agent": USER_AGENT}) as client:
        ...
        fallback = await fetch_hacker_news(client)
