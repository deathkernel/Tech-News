...
async def daily_brief(min_importance: int = 4) -> dict:
    ...
    return {
        ...
        "headline": f"{len(items)} technology signals worth tracking",
        "summary": "JARVIS filtered current technology updates for relevance, developer impact, security value, and future usefulness.",
        "stories": [
            {
                "title": item.title,
                "category": item.category,
